...
@registry.register_model("graph_attention_transformer_GeoTMI")
class GraphAttentionTransformerOC20(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_auxiliary_task and not self.use_attention_head:
            ...
            self.pos_dec_list = torch.nn.ModuleList()
            self.build_pos_dec(irreps_out_auxiliary,  auxiliary_head_dropout)
    ...
    def build_pos_dec(self, irreps_out_auxiliary,  auxiliary_head_dropout):
        for i in range(self.num_layers-1):
            pos_dec = GraphAttention(self.irreps_node_embedding, 
            self.irreps_node_attr, self.irreps_edge_attr, irreps_out_auxiliary,
            self.fc_neurons, 
            self.irreps_head, self.num_heads, self.irreps_pre_attn, 
            self.rescale_degree, self.nonlinear_message,
            alpha_drop=self.alpha_drop if auxiliary_head_dropout else 0.0, 
            proj_drop=0.0)
            self.pos_dec_list.append(pos_dec)
    ...
    def forward(self, data, pos_update):
            ...
            if idx < self.num_layers-1:
                ...
                if pos_update:
                    if self.use_auxiliary_task:
                        ...
                        #edge_src, edge_dst = edge_index[0], edge_index[1]
                        del edge_sh
                        del edge_length_embedding
                        torch.cuda.empty_cache()
                        
                        if self.use_pbc:
                            edge_vec = pos_.index_select(0, edge_src) - pos_.index_select(0, edge_dst) + offsets
                            dist = edge_vec.norm(dim=1)
                            edge_length = dist # ??? OC20 test doesn't use this
        ...
        if self.use_auxiliary_task:
            if pos_update:
                outputs_aux = self.auxiliary_head(node_input=node_features, 
                    ...
                    edge_attr=edge_sh, edge_scalars=edge_length_embedding, 
                    batch=batch)
                pos_ = pos_ + outputs_aux
                pos_list.append(pos_)
                return outputs, outputs_aux, pos_list
            else:
                return outputs, 0.0, pos_list
