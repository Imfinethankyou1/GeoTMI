...
from .base_trainer_GeoTMI import BaseTrainerV2, interpolate_init_relaxed_pos
...
@registry.register_trainer("energy_GeoTMI")
class EnergyTrainerV2(BaseTrainerV2):
    ...
    def train(self, disable_eval_tqdm=False):
        ...
        for epoch_int in range(
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                with torch.cuda.amp.autocast(enabled=self.scaler is not None):
                    out = self._forward(batch, True)
                    #self._compute_auxiliary_task_weight()
                    #loss = loss + loss_aux * weight
                    #self.current_auxiliary_task_weight
                    loss, pos_loss, ini_pos_loss = self._compute_loss(out, batch, weight=1.0)
                    for batch_data in batch:
                        ini_pos = batch_data.pos
                        #ini_pos_rel = batch_data.pos_relaxed
                        batch_data.pos = batch_data.pos_relaxed
                        #batch_data.ini_pos = ini_pos
                        #batch_data.ini_pos_rel = ini_pos_rel
                    if True:

                        #batch_ = deepcopy(batch)

                        out_rs = self._forward(batch, False)

                        for batch_data in batch:
                            batch_data.pos = ini_pos
                        loss_rs, _, _ = self._compute_loss(out_rs, batch,weight = 0.0)
                        #for batch_data in batch:
                        #    batch_data.pos_relaxed = batch_data.in_pos_rel
                        #    batch_data.pos = batch_data.ini_pos
                    loss = loss + loss_rs
                ...
                log_dict.update(
                    {
                        ...
                        "epoch": self.epoch,
                        "step": self.step,
                        'ini_pos_loss':ini_pos_loss.item(),
    ...
    def _forward(self, batch_list, pos_update=True):
        output = self.model(batch_list, pos_update)
        ...
        if not self.use_auxiliary_task:
            ...
            pass
            output_energy, output_pos, output_pos_list = output[0], output[1], output[2]
            if output_energy.shape[-1] == 1:
                output_energy = output_energy.view(-1)
            return {'energy': output_energy, 'pos': output_pos, 'pos_list': output_pos_list}
        

    def _compute_loss(self, out, batch_list, weight = 0.0):
        ...
        if self.use_auxiliary_task:
            ...
            self._compute_auxiliary_task_weight()
            count = 1
            if len(out['pos_list']) > 2:
                #print('Pos_list_run')
                for pred_pos in out['pos_list']:
                    pred_pos = pred_pos - pos
                    if self.normalizer.get("normalize_positions", False):
                        pred_pos = self.normalizers["positions"].norm(pred_pos)
                    loss_aux = self.loss_fn['force'](_mask_input(pred_pos, tag_mask), 
                        _mask_input(delta_pos*count/len(out['pos_list']), tag_mask))
                    #loss = loss + loss_aux * weight
        ...
        default_d_p = pos-pos
        if self.normalizer.get("normalize_positions", False):
           ddp  = self.normalizers["positions"].norm(default_d_p)
        ini_pos_loss = self.loss_fn['force'](_mask_input(ddp, tag_mask), _mask_input(delta_pos, tag_mask))
        return loss, loss_aux, ini_pos_loss
